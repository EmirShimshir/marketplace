...
// CreateWithoutCart provides a mock function with given fields: ctx, user
func (_m *UserRepository) CreateWithoutCart(ctx context.Context, user domain.User) (domain.User, error) {
	ret := _m.Called(ctx, user)

	if len(ret) == 0 {
		panic("no return value specified for CreateWithoutCart")
	}
